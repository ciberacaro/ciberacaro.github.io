...
- ✅ Toolchain at `tools/` (Python 3.8+ stdlib). Shared utilities in `tools/_lib.py` (includes `build_opener()` + `add_proxy_arg()` for Burp Suite / proxy routing). All tools support `--lang {en,pt}`, `--version`, and stdin via `-`. Networked tools additionally support `--user-agent STRING`; select tools also support `--proxy URL`. Uniform exit codes (0 ok / 1 issues / 2 usage / 3 network). 33 tools total — see `tools/README.md` for the canonical reference. Notable capabilities: `tls_inspect` enumerates accepted TLS versions (flags SSLv3 / TLS 1.0 / 1.1); `secrets_scan` Shannon-entropy gating + AI-token patterns (OpenAI / Anthropic / HuggingFace) + bare-assign entropy gate; `check_headers` redirect chains + proxy support; `subfinder` rate-limits DNS bursts (crt.sh + HackerTarget + wordlist); `dns_records` probes AXFR + queries CNAME/SOA; `cookie_check` validates `__Host-`/`__Secure-` prefix rules + long-expiry; `tech_fingerprint` detects WAFs; `xor_crack` for XOR ciphertext recovery (Cryptopals-grade); `path_scan` gobuster-style with wildcard/soft-404 auto-detection and body-size filtering; `subdomain_takeover` for dangling-CNAME detection; `log_parser` Apache/syslog/JSON-log (CloudWatch, GCP, Docker/ECS); `email_forensics`, `file_hash` for forensics work (mapped to UC01481/82/85/89 of the Portuguese CET Cybersecurity curriculum); `open_redirect` (8 payloads × 28 params); `param_miner` (hidden parameter discovery); `crlf_inject` (HTTP response splitting via raw http.client, 8 payload variants incl. double-encoded); `ssrf_probe` (29 internal/cloud-metadata payloads); `http_smuggling_probe` (CL.TE / TE.CL / TE.TE timing side-channel); `xxe_probe` (entity + XInclude + param-entity timing, 6 payloads).
  ...
  - `path_scan.py` — wordlist-based HTTP path/directory discovery (gobuster-style, threaded, risk-flagged, wildcard/soft-404 auto-detection).
