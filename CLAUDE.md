...
## Current build state (last updated 2026-05-20, Session 5 complete — Phase 2)
...
- ✅ Toolchain at `tools/` (Python 3.8+ stdlib). Shared utilities in `tools/_lib.py` (includes `build_opener()` + `add_proxy_arg()` for Burp Suite / proxy routing). All tools support `--lang {en,pt}`, `--version`, and stdin via `-`. Networked tools additionally support `--user-agent STRING`; select tools also support `--proxy URL`. Uniform exit codes (0 ok / 1 issues / 2 usage / 3 network). 33 tools total — see `tools/README.md` for the canonical reference. Notable capabilities: `tls_inspect` enumerates accepted TLS versions (flags SSLv3 / TLS 1.0 / 1.1); `secrets_scan` Shannon-entropy gating + AI-token patterns + severity grouping (critical→info) + .env filename detection; `check_headers` redirect chains + proxy support + deep CSP analysis (data: URI, http: scheme, missing object-src); `subfinder` rate-limits DNS bursts (crt.sh + HackerTarget + wordlist); `dns_records` probes AXFR + queries CNAME/SOA; `cookie_check` validates `__Host-`/`__Secure-` prefix rules + long-expiry; `tech_fingerprint` detects WAFs + extracts version numbers (nginx, Apache, PHP, WordPress, jQuery, etc.); `cors_check` severity-split: reflected+ACAC=true → critical (full session theft); `recon` orchestrator with opt-in `--path-scan`/`--path-preset` per host; `xor_crack` for XOR ciphertext recovery (Cryptopals-grade); `path_scan` gobuster-style with `--preset quick` (75 paths) / `--preset medium` (263 paths: CMS, CI/CD, API docs, CTF staples) + wildcard/soft-404 auto-detection; `subdomain_takeover` for dangling-CNAME detection; `log_parser` Apache/syslog/JSON-log (CloudWatch, GCP, Docker/ECS); `email_forensics`, `file_hash` for forensics work (mapped to UC01481/82/85/89 of the Portuguese CET Cybersecurity curriculum); `open_redirect` (8 payloads × 28 params); `param_miner` (hidden parameter discovery + `--bool-probe` for boolean-gated flags); `crlf_inject` (HTTP response splitting via raw http.client, 8 payload variants incl. double-encoded); `ssrf_probe` (29 internal/cloud-metadata payloads); `http_smuggling_probe` (CL.TE / TE.CL / TE.TE timing side-channel); `xxe_probe` (entity + XInclude + param-entity timing, 6 payloads).
  ...
  - `multidecode.py` — auto-decode Base64/Base32/Base85/hex/URL/HTML entities/Unicode escapes (`\uXXXX`/`\u{XXXX}`/`\xXX`)/binary/ROT13, with `--cascade`.
  ...
  - `cors_check.py` — probe with attacker/null/prefix/suffix Origins; severity-split: reflected+ACAC=true → `reflected_with_credentials` (critical); reflected without creds → medium; wildcard+creds / null acceptance.
  - `subfinder.py` — crt.sh + HackerTarget + DNS wordlist subdomain enumeration (`--skip-hackertarget` flag).
  ...
  - `path_scan.py` — wordlist-based HTTP path/directory discovery (gobuster-style, threaded, risk-flagged, `--preset quick` 75 paths / `--preset medium` 263 paths, wildcard/soft-404 auto-detection).
  - `subdomain_takeover.py` — dangling-CNAME subdomain takeover detection (16 service fingerprints, crt.sh enum).
  - `log_parser.py` — log normalisation + brute-force/scanner detection (Apache/nginx/syslog/JSON-log CloudWatch+GCP+Docker, streamed line-by-line).
  ...
  - `param_miner.py` — discover hidden query parameters (278-name wordlist, baseline delta + reflection, threaded; GET + POST form/json modes, `--wordlist`, `--bool-probe` for boolean-gated feature flags).
  - `crlf_inject.py` — HTTP response splitting / CRLF injection (http.client raw, canary-header confirmed, 8 payload variants incl. double-encoded `%250d%250a` for WAF bypass, `--max-params N`).
  ...
  - `xxe_probe.py` — XXE injection probe (6 payloads: entity file read /etc/passwd + C:/Windows/win.ini, SSRF AWS IMDSv1 + GCP metadata, XInclude bypass, param-entity blind SSRF timing; `--proxy` support).
