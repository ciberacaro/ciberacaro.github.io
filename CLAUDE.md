...
- ✅ Toolchain at `tools/` (Python 3.8+ stdlib). Shared utilities in `tools/_lib.py`. All tools support `--lang {en,pt}`, `--version`, and stdin via `-`. Networked tools additionally support `--user-agent STRING`. Uniform exit codes (0 ok / 1 issues / 2 usage / 3 network). 21 tools total — see `tools/README.md` for the canonical reference. Notable capabilities after the 2026-05-18 hardening: `tls_inspect` enumerates accepted TLS versions (flags SSLv3 / TLS 1.0 / 1.1 if any are enabled); `secrets_scan` has Shannon-entropy gating on generic patterns; `check_headers` shows redirect chains; `subfinder` rate-limits DNS bursts.
