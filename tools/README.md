...
| `--type` | `box`, `ctf`, `bugbounty` | `box` | Template: box (Recon/Foothold/PrivEsc), ctf (Approach/Solution/Flag), bugbounty (PoC/Impact/Remediation/Timeline). |
| `--platform` | `thm`, `htb`, `portswigger`, `other` | `thm` | |
| `--difficulty` | `info`, `easy`, `medium`, `hard`, `insane` | `easy` | Only used for `--type box`. |
| `--tags` | comma-separated string | empty | Extra tags appended to `[platform, type]`. |
...
tools/multidecode.py "&lt;script&gt;alert(1)&lt;/script&gt;"   # HTML entities
```
...
Fetch `/robots.txt` and `/sitemap.xml`. Lists rules per User-Agent, highlights paths matching interesting patterns (admin, api, backup, debug, .git, etc.), and prints the first 50 sitemap URLs as a recon starting point. Automatically follows `<sitemapindex>` (nested sitemaps) up to depth 2, up to 5 per level.
...
Identify the likely type(s) of a hash string. Knows ~27 formats: MD5/SHA-family, bcrypt, scrypt, Argon2, PBKDF2 (Django and PHC formats), phpass (WordPress), Apache APR1, Unix crypt variants ($1$/$5$/$6$/$y$), MySQL old/new, NTLM/LM pwdump format, base64-encoded LDAP forms, and JWT (commonly mistaken for a hash). Ranks candidates by confidence and includes hashcat mode hints.
...
Decode a JSON Web Token (header / payload / signature), pretty-print claims with human-readable timestamps and validity windows, and flag common issues: `alg: none`, weak algorithms, missing `exp`, expired tokens, validity windows >24h, missing `iss`/`aud`, path-traversal and null-byte patterns in `kid`, and HS256/384/512 tokens signed with an empty secret (attempts HMAC verification with key `""`).
...
tools/crlf_inject.py "https://example.com/?a=1&b=2&c=3&d=4" --max-params 4
```

## `ssrf_probe.py`

Probe URL parameters for Server-Side Request Forgery. Tests 29 internal payloads (127.0.0.1, AWS IMDSv1 at 169.254.169.254, GCP metadata, Azure, Digital Ocean, common internal IPs/ports, IP aliases 0.0.0.0/0x7f000001/decimal, and protocol payloads file:///etc/passwd, dict://, gopher://) against URL-accepting parameters (`url`, `src`, `redirect`, `fetch`, `proxy`, …). Flags responses where the server took significantly longer than baseline (timing side-channel) or returned an unexpected 200/4xx from an internal address.
...
Detect HTTP/1.1 request smuggling susceptibility (CL.TE, TE.CL, and TE.TE obfuscation desync) via timing side-channel. Sends requests with deliberately contradictory `Content-Length` / `Transfer-Encoding` headers using raw sockets (no normalization). A vulnerable back-end stalls waiting for bytes that never arrive, producing a measurable delay against a baseline GET. Reports all three technique variants independently.
...
Analyse `Set-Cookie` response headers. Flags missing HttpOnly on session-looking names, missing Secure over HTTPS, missing SameSite, SameSite=None without Secure, Partitioned (CHIPS) without Secure, Domain scoped wider than the response host, RFC 6265bis `__Host-` / `__Secure-` prefix violations, and long-lived cookies (Max-Age or Expires > 1 year).
...
Pure-stdlib DNS client (raw UDP/53 with TCP fallback per RFC 1035 §4.2.2). Queries A/AAAA/MX/NS/CNAME/SOA/TXT/CAA and analyses email auth: SPF (missing, multiple, `~all`/`?all`), DMARC (missing, `p=none`), DKIM (tries 8 selectors: default, google, k1, s1, dkim, mail, selector1, selector2), CAA (missing).
...
Wayback Machine snapshot lookup via the `archive.org` availability and CDX APIs. `--timeline` lists historical snapshots; `--diff` fetches the closest snapshot and unified-diffs it against the live URL. Dynamic content (CSRF tokens, nonces, timestamps, long hex strings) is filtered before diffing to reduce noise. `--max-diff N` caps the diff output (default 100 lines). Snapshot body fetch is capped at 500 KB.
